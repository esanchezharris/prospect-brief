...
from .models import Brief, BriefSection, Document, Evidence, RunReport
...
async def run_brief(
    *, subject: str, anchors: dict[str, str], institution: str, config: Config, llm: LLMProvider, search: SearchProvider,
    run_dir: Path, transport: httpx.AsyncBaseTransport | None = None, log=print,
) -> tuple[Brief, Path]:
    t0 = time.monotonic()
    ...
    # 5. verify (code)
    verify_evidence(evidence, {d.cache_key: d for d in docs}, config, subject)
    n_ver = sum(1 for e in evidence if e.status == "verified")
    report.counts["claims_verified"] = n_ver
    report.counts["claims_dropped"] = len(evidence) - n_ver
    log(f"[verify] {n_ver} verified, {len(evidence) - n_ver} dropped")
    ...
    brief = Brief(subject=subject, institution=institution, anchors=anchors, generated_at=report.finished_at, sections=draft.sections, talking_points=draft.talking_points, evidence=evidence, gaps=gaps, report=report)
    summary_line = (
        f"{len(evidence)} claims extracted, {n_ver} verified, {len(evidence) - n_ver} dropped, "
